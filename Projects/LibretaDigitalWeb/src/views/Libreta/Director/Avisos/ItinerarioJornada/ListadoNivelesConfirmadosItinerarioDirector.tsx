...
const ListadoNivelesConfirmadosItinerarioDirector = () => {
  ...
  useEffect(() => {
    const fetchNiveles = async () => {
      setLoading(true);
      if (!isLoading && idItinerario) {
        try {
          const nivelesData = await itinerarioController.getNivelesByItinerario(
            +idItinerario
          );
          if (nivelesData) {
            setNiveles(
  ...
  const handleNivelClick = (idNivel: number) => {
    if (idItinerario) {
      navigate(
        `${initPathName}/avisos/itinerario-jornada/revisar-listado-menores/${+idItinerario}/${idNivel}`
      );
    }
  };
        ...
        <div className="grid gap-2">
          {filteredNiveles.length > 0 ? (
            filteredNiveles.map((nivel) => (
              <div
                key={nivel.idNivel + "-" + idItinerario}
...
export default ListadoNivelesConfirmadosItinerarioDirector;
