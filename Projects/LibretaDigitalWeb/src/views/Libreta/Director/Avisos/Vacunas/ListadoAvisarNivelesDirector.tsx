...
const ListadoAvisarNivelesDirector = () => {
  ...
  useEffect(() => {
    const fetchNiveles = async () => {
      ...
      if (!isLoading) {
        try {
          const nivelesData =
            await vacunaController.getAllNivelesAvisarVacuna();
          if (nivelesData) {
            setNiveles(
              nivelesData.map(
                (nivel: {
                  ...
                  id: nivel.id,
                  nivel: nivel.nivel,
                  nmroSolicitados: nivel.nmroSolicitados,
                  nmroNoSolicitados: nivel.nmroNoSolicitados,
                  fechaVacuna: nivel.fechaVacuna,
                  solicitado: nivel.solicitado,
                })
              )
            );
          }
  ...
  const handleNivelClick = (idNivel: number) => {
    navigate(`${initPathName}/avisos/vacunas/solicitar-vacunas/${idNivel}`);
  };

  const filteredNiveles = niveles.filter((nivel) =>
    nivel.nivel.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="grid gap-2">
          {filteredNiveles.length > 0 ? (
            filteredNiveles.map((nivel) => (
              ...
              >
                <h2 className="font-bold">{nivel.nivel}</h2>
                <p>
                  <span className="font-semibold">Solicitados:</span>{" "}
                  {nivel.nmroSolicitados}
                </p>
                <p>
                  <span className="font-semibold">No Solicitados:</span>{" "}
                ...
                {nivel.solicitado ? (
                  ...
                  <p className="text-red-700 font-bold">VACUNA NO SOLICITADA</p>
                )}
              </div>
            ))
          ) : (
...
export default ListadoAvisarNivelesDirector;
