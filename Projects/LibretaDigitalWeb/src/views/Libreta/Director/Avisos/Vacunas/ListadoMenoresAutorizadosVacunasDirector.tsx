...
const ListadoMenoresAutorizadosVacunasDirector = () => {
  ...
  useEffect(() => {
    const fetchMenores = async () => {
      ...
      if (!isLoading && idNivel) {
        try {
          const menoresData = await nivelController.getMenoresByNivelDirector(+idNivel);
          if (menoresData) {
            setMenores(
              menoresData.map(
                (menor: {
...
export default ListadoMenoresAutorizadosVacunasDirector;
